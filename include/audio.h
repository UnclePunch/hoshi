...
static BGMData *stc_bgm_data = 0;                                // voice index of the bgm currently playing, is -1 when nothing is playing
static VPB *stc_bgm_vpb = 0;                                     // is set when the first hps sample is played @ 8038b33c. value is copied to the prev member of the vpb struct before being updated
static int *stc_bgm_data_seed = 0;                               // is used to generate the next bgm's bgm_data
static int *stc_bgm_tick = 0;                                    // how many times bgm audio has been updated (incremented @ 8038ad44)
static BGMData *stc_bgm_data2 = 0;                               // contains some data about the bgm, including its voice_index
static int *stc_bgm_entrynum = 0;                                // entrynum of the hps file currently playing again
static int *stc_bgm_is_playing = 0;                              // is -1 when not playing
static int *stc_bgm_curLoadingHpsChunkHeaderIndex = 0;           // index of the cur hps chunk header loaded (0-2 circular buffer)
static int *stc_bgm_lastLoadedHPSChunkIndex = 0;                 // index of the most recent hps chunk loaded (0-2 circular buffer), changes after hps chunk is loaded @ 8038ad6c
static int *stc_bgm_curPlayingHpsChunkHeaderIndex = 0;           // index of the cur hps chunk header playing (0-2 circular buffer)
static u8 *stc_bgm_isLoadingHPSChunk = 0;                        // flag that indicates an hps chunk is being loaded
static int *stc_bgm_aramAlloc = 0;                               // start of the 3 hps chunk circular buffer
static int *stc_fgm_tick = 0;                                    // how many times fgm audio has been updated (incremented @ 8038ad44)
static FGMLive *stc_last_fgmlive = 0;                            // points to the most recently created FGMLive struct
